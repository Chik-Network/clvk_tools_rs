...
pip_installed_version=$(pip list | grep clvk_tools_rs | awk '{print $2}')
python_import_version=$(python -c 'import clvk_tools_rs; print(clvk_tools_rs.get_version())')
...
if [ "$expected_version" == "$pip_installed_version" ] && [ "$expected_version" == "$python_import_version" ]; then
    ...
    echo "clvk_tools_rs VERSIONS does not match expected version"
