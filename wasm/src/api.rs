...
use clvk_tools_rs::classic::clvk::__type_compatibility__::{
    Bytes, Stream, UnvalidatedBytesFromType,
};
use clvk_tools_rs::classic::clvk::serialize::sexp_to_stream;
use clvk_tools_rs::classic::clvk_tools::clvkc::compile_clvk_inner;
use clvk_tools_rs::classic::clvk_tools::stages::stage_0::DefaultProgramRunner;
use clvk_tools_rs::compiler::cldb::{
    hex_to_modern_sexp, CldbOverrideBespokeCode, CldbRun, CldbRunEnv, CldbRunnable,
    CldbSingleBespokeOverride,
};
use clvk_tools_rs::compiler::clvk::{convert_to_clvk_rs, start_step};
use clvk_tools_rs::compiler::compiler::{
    extract_program_and_env, path_to_function, rewrite_in_program, DefaultCompilerOpts,
};
use clvk_tools_rs::compiler::comptypes::{CompileErr, CompilerOpts};
use clvk_tools_rs::compiler::prims;
use clvk_tools_rs::compiler::repl::Repl;
use clvk_tools_rs::compiler::runtypes::RunFailure;
...
fn create_clvk_runner_run_failure(err: &RunFailure) -> JsValue {
    match err {
        RunFailure::RunErr(l, e) => create_clvk_runner_err(format!("{}: Error {}", l, e)),
        RunFailure::RunExn(l, e) => create_clvk_runner_err(format!("{}: Exn {}", l, e)),
    }
}

fn create_clvk_compile_failure(err: &CompileErr) -> JsValue {
    match err {
        CompileErr(l, e) => create_clvk_runner_err(format!("{}: Error {}", l, e)),
    }
}
