...
```bash
# Make sure you're at <clvk_tools_rs root>/wasm
...
Prerequisite:
- NodeJS >= 18
- Wasm files built by `wasm-pack` command exist at `<clvk_tools_rs root>/wasm/pkg/`
...
Program is exported by ```clvk_tools_rs``` and contains a ```to``` function
