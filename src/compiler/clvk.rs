...
use crate::classic::clvk_tools::stages::stage_0::{RunProgramOption, TRunProgram};
...
/// An object which contains the state of a running CLVK program in a compact
/// form.
///
/// Being immutable, it can be preserved, examined and compared as desired.  The
/// whole record of CLVK execution can be observed by collecting these until the
...
pub enum RunStep {
    ...
    /// A step about to be taken.  Indicates a clvk expression and env, plus the
    ...
    match sexp.borrow() {
        ...
        SExp::Cons(_l, _a, nil) => match nil.borrow() {
            ...
            _ => Err(RunFailure::RunErr(
                sexp.loc(),
                format!("Unexpected head form in clvk {sexp}"),
            )),
        },
    }
}
...
/// Given an SExp, produce a clvkr style NodePtr in the given allocator.
pub fn convert_to_clvk_rs(
    allocator: &mut Allocator,
    head: Rc<SExp>,
    ...
    match head.borrow() {
        ...
        SExp::Cons(_, a, b) => convert_to_clvk_rs(allocator, a.clone()).and_then(|head_ptr| {
            convert_to_clvk_rs(allocator, b.clone()).and_then(|tail| {
...
/// Given an allocator and clvkr NodePtr, produce an SExp which is equivalent.
pub fn convert_from_clvk_rs(
    allocator: &mut Allocator,
    loc: Srcloc,
    head: NodePtr,
) -> Result<Rc<SExp>, RunFailure> {
    match allocator.sexp(head) {
        ...
        allocator::SExp::Pair(a, b) => {
            convert_from_clvk_rs(allocator, loc.clone(), a).and_then(|h| {
                convert_from_clvk_rs(allocator, loc.clone(), b)
...
#[test]
fn test_convert_from_clvk_rs_00_byte() {
    let mut allocator = Allocator::new();
    let allocator_atom = allocator.new_atom(&[0]).expect("should convert");
    let srcloc = Srcloc::start("*test*");
    let result = convert_from_clvk_rs(&mut allocator, srcloc.clone(), allocator_atom)
    ...
    let node = convert_to_clvk_rs(&mut allocator, result).expect("should convert from mod");
    let atom = allocator.atom(node);
    assert_eq!(atom.as_ref(), &[0]);
}

#[test]
fn test_convert_to_clvk_rs_m129() {
    let mut allocator = Allocator::new();
    let srcloc = Srcloc::start("*test*");
...
fn apply_op(
    ...
    let converted_app = convert_to_clvk_rs(allocator, application.clone())?;
    let converted_args = convert_to_clvk_rs(allocator, wrapped_args.clone())?;
    ...
    runner
        ...
        .and_then(|v| convert_from_clvk_rs(allocator, head.loc(), v.1))
}
...
/// Use the RunStep object to evaluate some clvk to completion.
pub fn run(
    allocator: &mut Allocator,
    runner: Rc<dyn TRunProgram>,
    prim_map: Rc<HashMap<Vec<u8>, Rc<SExp>>>,
...
/// parses the clvk text and runs to completion.
